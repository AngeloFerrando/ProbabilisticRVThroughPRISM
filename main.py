...
def main():
    ...
    if not big:
        ...
        os.system('prism ' +  model_file.replace('.prism', '_instr.prism') + ' -exportstates ' +  model_file.replace('.prism', '_instr.sta') + ' -exporttrans ' +  model_file.replace('.prism', '_instr.tra') + ' -exportlabels ' +  model_file.replace('.prism', '_instr.lab'))
        ...
        os.system('bigrapher/bigrapher full -p ' +  model_file.replace('.big', '.tra') + ' -l ' +  model_file.replace('.big', '.csl') + ' --solver=MCARD -M 5000 ' +  model_file)
    ...
    with open(trace_file, 'r') as file:
        for line in file.readlines():
            line = line.replace('\n', '')
            print('EVENT:', line)
            print(monitor.next(set(line.split(','))))
